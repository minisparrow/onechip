...
class Core extends Module {
  ...
  val csignals = ListLookup(inst,
    ...
    Array(
      ...
      VSETVLI -> List(ALU_X     , OP1_X  , OP2_X  , MEN_X, REN_S, WB_VL   , CSR_V), // 追加
		)
	)
  ...
  br_flg := MuxCase(false.B, Seq(
    ...
    (exe_fun === BR_BLT)  ->  (op1_data.asSInt() < op2_data.asSInt()).asUInt(),
    (exe_fun === BR_BGE)  -> !(op1_data.asSInt() < op2_data.asSInt()).asUInt(),
    (exe_fun === BR_BLTU) ->  (op1_data.asSInt() < op2_data.asSInt()).asUInt(),
    (exe_fun === BR_BGEU) -> !(op1_data.asSInt() < op2_data.asSInt()).asUInt()
  ))
