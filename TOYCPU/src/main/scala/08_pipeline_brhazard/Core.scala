...
class Core extends Module {
  ...
  val id_rs1_data = Mux((id_rs1_addr =/= 0.U), regfile(id_rs1_addr), 0.U(WORD_LEN.W))
  val id_rs2_data = Mux((id_rs2_addr =/= 0.U), regfile(id_rs2_addr), 0.U(WORD_LEN.W))
  ...
  exe_alu_out := MuxCase(0.U(WORD_LEN.W), Seq(
    (exe_reg_exe_fun === ALU_ADD)   -> (exe_reg_op1_data + exe_reg_op2_data)(31,0), // オーバーフロー切り捨て
